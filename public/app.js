const state = { user: null, page: "dashboard", events: [], registrations: [], payments: [], schedules: [], profile: null, adminTab: "payments", selectedCourt: null, selectedSlotDate: "", selectedSlotIds: [], currentSlots: [], courtReviews: [], canReview: false, myReview: null };
const $ = (selector) => document.querySelector(selector);
const $$ = (selector) => [...document.querySelectorAll(selector)];
const esc = (value) => String(value ?? "").replace(/[&<>"']/g, (char) => ({ "&":"&amp;", "<":"&lt;", ">":"&gt;", '"':"&quot;", "'":"&#039;" }[char]));
const initials = (name) => (name || "PB").split(" ").map((part) => part[0]).slice(0, 2).join("").toUpperCase();
const money = (amount) => `₱${Number(amount || 0).toLocaleString("en-PH", { minimumFractionDigits: Number(amount || 0) % 1 ? 2 : 0 })}`;
const dateText = (date, options = { month: "short", day: "numeric", year: "numeric" }) => new Date(date).toLocaleDateString("en-PH", options);
...
const courtRules = (court) => {
  try { return Array.isArray(court.rate_rules) ? court.rate_rules : JSON.parse(court.rate_rules || "[]"); } catch { return []; }
};
const courtAmenities = (court) => {
  try { return Array.isArray(court.amenities) ? court.amenities : JSON.parse(court.amenities || "[]"); } catch { return []; }
};
const courtPriceRange = (court) => {
  const prices = courtRules(court).map((rule) => Number(rule.price)).filter(Number.isFinite);
...
function applyBranding(root = document.body) {
  ...
  while ((node = walker.nextNode())) {
    node.nodeValue = node.nodeValue.replaceAll("RallyPoint", "PickleBalls").replaceAll("RALLYPOINT", "PICKLEBALLS");
  }
}
applyBranding();
...
function setBusy(button, busy, label = "Working…") {
  if (!button) return;
  if (busy) { button.dataset.original = button.innerHTML; button.disabled = true; button.innerHTML = `<span class="spinner"></span>${label}`; }
...
function switchAuth(tab) {
  ...
  $$(".auth-menu-item").forEach((button) => button.classList.toggle("active", button.dataset.authTab === tab));
  $$(".auth-panel").forEach((panel) => panel.classList.toggle("active", panel.id === `${tab}-panel`));
  $(".auth-tabs").classList.toggle("hidden", tab.startsWith("admin") || tab === "forgot" || tab === "reset");
}
...
async function handleAuth(event, type) {
  ...
  try {
    ...
    const data = await api(`/api/auth/${endpoint}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (data.pendingApproval) { switchAuth(type === "admin-register" ? "admin" : "login"); toast(data.message, "success"); return; }
    state.user = data.user;
    await bootApp();
    toast(type === "admin" ? "Welcome to the admin panel." : type === "admin-register" ? "Admin access request submitted." : type === "login" ? "Welcome back to PickleBalls." : "Your PickleBalls account is ready.");
...
function updateUserChrome() {
  ...
  $("#sidebar-role").textContent = state.user?.role === "admin" ? "Administrator" : "Member";
...
async function navigate(page) {
  ...
  $("#breadcrumb-title").textContent = page === "payment-history" ? "Payment History" : page.replace("-", " ").replace(/\b\w/g, (char) => char.toUpperCase());
  $(".sidebar")?.classList.remove("open");
  $("#page-container").innerHTML = `<div class="loading"><span class="spinner"></span>Loading PickleBalls…</div>`;
  const pages = { dashboard: renderDashboard, profile: renderProfile, events: renderEvents, registrations: renderRegistrations, schedule: renderSchedule, payments: renderPayments, "payment-history": renderPaymentHistory, notifications: renderNotifications, settings: renderSettings, admin: openAdmin };
  try { await (pages[page] || renderDashboard)(); } catch (error) { $("#page-container").innerHTML = `<div class="page"><div class="empty-state"><div>◌</div><strong>Could not load this page</strong><p>${esc(error.message)}</p></div></div>`; toast(error.message, "error"); }
  if (window.innerWidth <= 720) $(".sidebar")?.classList.remove("open");
}

async function renderDashboard() {
  const [dashboard, events, registrations, notifications] = await Promise.all([api("/api/dashboard"), api("/api/events"), api("/api/registrations"), api("/api/notifications")]);
  state.events = events.events; state.registrations = registrations.registrations;
  ...
  $("#top-notification-badge").classList.toggle("hidden", !dashboard.unreadNotifications);
  $("#page-container").innerHTML = `<div class="page">
    ...
    <div class="dashboard-grid"><div class="card"><div class="card-title"><h3>Next on your calendar</h3><button class="text-link" data-page="registrations">View all →</button></div>${next ? `<div class="next-event"><span class="event-status">Registered</span><p class="eyebrow">UPCOMING EVENT</p><h3>${esc(next.name)}</h3><div class="event-meta"><span>◷ ${dateText(next.event_date)}</span><span>⌖ ${esc(next.location)}</span></div></div>` : `<div class="empty-state"><div>✦</div><strong>No games booked yet</strong><p>Find an event and save your first spot.</p><button class="button primary small" data-page="events" style="margin-top:15px">Browse events</button></div>`}</div>
      <div class="card"><div class="card-title"><h3>Recent activity</h3><button class="text-link" data-page="notifications">See updates →</button></div><div class="activity-list">${notifications.notifications.slice(0, 3).map((notification) => `<div class="activity"><div class="activity-dot">${notification.type === "success" ? "✓" : "◌"}</div><div><p>${esc(notification.title)}</p><small>${esc(notification.message).slice(0, 72)}${notification.message.length > 72 ? "…" : ""}</small></div></div>`).join("") || `<div class="empty-state"><strong>All quiet here</strong><p>Your updates will show up here.</p></div>`}</div></div></div>
  </div>`;
}

async function renderEvents() {
  const data = await api("/api/events"); state.events = data.events;
  $("#page-container").innerHTML = `<div class="page"><div class="page-heading"><div><p class="eyebrow">FIND YOUR NEXT COURT</p><h1>Available courts</h1><p>Browse open playing spaces, see the details, and request your spot.</p></div><div class="heading-actions"><span class="date-chip">${data.events.length} available</span></div></div>
  ...
  $("#category-filter").addEventListener("change", filterEvents); $("#event-search").addEventListener("input", filterEvents);
}

function courtCard(event, index = state.events.indexOf(event)) {
  const meta = [
    { surface: "Sport Court", type: "Indoor", hours: "8:00 AM–11:00 PM", amenities: ["Parking", "Restrooms", "Lights"] },
    { surface: "Pickleball Court", type: "Outdoor", hours: "7:00 AM–10:00 PM", amenities: ["Seating", "Lights", "Parking"] },
    { surface: "Sport Court", type: "Indoor", hours: "6:00 AM–12:00 AM", amenities: ["Restrooms", "Food & coffee", "Lights"] },
  ][Math.abs(index) % 3];
  ...
  const opening = event.opening_time ? clockText(event.opening_time) : meta.hours.split("–")[0];
  const closing = event.closing_time ? clockText(event.closing_time) : meta.hours.split("–")[1];
  const image = event.image_url ? `<img src="${esc(event.image_url)}" alt="${esc(event.name)} court" loading="lazy">` : "";
  const reviews = Number(event.review_count || 0);
  const rating = reviews ? `★ ${Number(event.rating || 0).toFixed(1)} <small>(${reviews})</small>` : "No ratings yet";
  const bookingLabel = full ? `<button class="button ghost small view-court" data-id="${event.id}">View details</button>` : `<button class="button primary small view-court" data-id="${event.id}">Choose your slot ↗</button>`;
  return `<article class="court-card event-card" data-category="${esc(event.category)}" data-search="${esc(`${event.name} ${event.location}`.toLowerCase())}">
    ...
    <div class="court-card-body">
      ...
      <div class="court-amenities">${displayAmenities.slice(0, 4).map((amenity) => `<span>${esc(amenity)}</span>`).join("")}</div>
...
async function showCourtDetails(courtId, date = localDate()) {
  ...
  state.currentSlots = [];
  $("#page-container").innerHTML = `<div class="loading"><span class="spinner"></span>Loading court details…</div>`;
  try {
    ...
    $("#page-container").innerHTML = `<div class="page"><div class="empty-state"><div>◌</div><strong>Could not load this court</strong><p>${esc(error.message)}</p><button class="button ghost small" data-page="events">Back to courts</button></div></div>`;
...
function renderCourtDetails() {
  ...
  const opening = court.opening_time ? clockText(court.opening_time) : "7:00 AM";
  const closing = court.closing_time ? clockText(court.closing_time) : "11:00 PM";
  const image = court.image_url ? `<img src="${esc(court.image_url)}" alt="${esc(court.name)} court">` : "";
  const today = localDate();
  const nowMinutes = new Date().getHours() * 60 + new Date().getMinutes();
  const slotCells = slots.map((slot) => {
    const startMinutes = Number(String(slot.start_time).slice(0, 2)) * 60 + Number(String(slot.start_time).slice(3, 5));
    const past = state.selectedSlotDate < today || (state.selectedSlotDate === today && startMinutes <= nowMinutes);
  ...
  const selectedSlots = slots.filter((slot) => state.selectedSlotIds.includes(slot.id));
  const total = selectedSlots.reduce((sum, slot) => sum + Number(slot.price), 0);
  const reviewCount = Number(court.review_count || 0);
  const ratingLabel = reviewCount ? `★ ${Number(court.rating || 0).toFixed(1)} <span>(${reviewCount} reviews)</span>` : `<span class="unrated">No ratings yet</span>`;
  const reviewForm = state.canReview ? `<form id="court-review-form" class="review-form">
  ...
  </form>` : `<div class="review-eligibility">${state.user?.role === "member" ? "Book this court and wait for approval before rating it." : "Sign in as a member with an approved booking to rate this court."}</div>`;
  const reviewList = state.courtReviews.length ? state.courtReviews.map((review) => `<article class="court-review"><div class="review-avatar">${initials(review.full_name)}</div><div class="review-copy"><div class="review-meta"><strong>${esc(review.full_name)}</strong><span>${"★".repeat(Number(review.rating))}${"☆".repeat(5 - Number(review.rating))}</span></div><p>${review.comment ? esc(review.comment) : "No written comment."}</p><small>${dateText(review.updated_at || review.created_at, { month: "short", day: "numeric", year: "numeric" })}${review.updated_at && review.created_at !== review.updated_at ? " · Edited" : ""}</small></div></article>`).join("") : `<div class="empty-review-state">No player ratings yet. Be the first approved player to share your experience.</div>`;
  ...
  reviewFormElement?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      toast(state.myReview ? "Your review was updated." : "Thanks for rating this court.");
...
function updateBookingSummary() {
  ...
  const selected = state.currentSlots.filter((slot) => state.selectedSlotIds.includes(slot.id));
  list.innerHTML = selected.length ? selected.map((slot) => `<div class="selected-slot-line"><span>${clockText(slot.start_time)}–${clockText(slot.end_time)}</span><strong>${money(slot.price)}</strong></div>`).join("") : `<div class="summary-empty">No slots selected yet.</div>`;
  totalElement.textContent = money(selected.reduce((sum, slot) => sum + Number(slot.price), 0));
  confirmButton.disabled = !selected.length;
}

function eventCard(event) {
  const colors = { Tournament: "", Clinic: "blue", Community: "orange", Social: "" };
  const full = Number(event.available_slots) < 1;
...
function filterEvents() {
  const category = $("#category-filter").value.toLowerCase(); const search = $("#event-search").value.toLowerCase();
  $$(".event-card").forEach((card) => card.classList.toggle("hidden", (category && card.dataset.category.toLowerCase() !== category) || (search && !card.dataset.search.includes(search))));
}

async function renderRegistrations() {
  const data = await api("/api/registrations"); state.registrations = data.registrations;
  $("#page-container").innerHTML = `<div class="page"><div class="page-heading"><div><p class="eyebrow">YOUR COURT TIME</p><h1>My bookings</h1><p>Every court you’ve requested, all in one place.</p></div><button class="button lime small" data-page="events">Browse courts ↗</button></div><div class="card table-card"><table class="data-table"><thead><tr><th>Court</th><th>Date & location</th><th>Booking</th><th>Payment</th><th></th></tr></thead><tbody>${data.registrations.map((item) => `<tr><td><strong>${esc(item.name)}</strong><br><span style="font-size:10px;color:#9aa69f">${esc(item.category)}</span></td><td>${dateText(item.event_date)}<br><span style="font-size:10px;color:#9aa69f">${esc(item.location)}</span></td><td>${statusPill(item.status)}</td><td>${item.payment_status ? statusPill(item.payment_status) : `<span class="pill draft">${item.status === "confirmed" ? "Not submitted" : "Awaiting approval"}</span>`}</td><td>${item.status === "confirmed" && (!item.payment_status || item.payment_status === "rejected") ? `<button class="button ghost small pay-for-event" data-id="${item.id}">Pay now</button>` : ""}</td></tr>`).join("") || `<tr><td colspan="5"><div class="empty-state"><strong>No bookings yet</strong><p>Browse courts to request your next spot.</p></div></td></tr>`}</tbody></table></div></div>`;
}

async function renderSchedule() {
  const data = await api("/api/schedules"); state.schedules = data.schedules;
  $("#page-container").innerHTML = `<div class="page"><div class="page-heading"><div><p class="eyebrow">PLAN YOUR COURT TIME</p><h1>My schedule</h1><p>Save the days and times you plan to play so your court calendar stays organized.</p></div></div>
...
function showAccountModal() {
  const modal = document.createElement("div"); modal.className = "modal-backdrop"; modal.id = "account-modal";
  ...
  $("#modal-root").append(modal); modal.querySelectorAll(".modal-close").forEach((button) => button.addEventListener("click", () => modal.remove()));
  $("#account-form").addEventListener("submit", async (event) => {
    event.preventDefault(); const button = event.target.querySelector("button[type=submit]"); setBusy(button, true, "Creating…");
    try { await api("/api/admin/users", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); modal.remove(); toast("Account created."); await openAdmin(); }
    catch (error) { toast(error.message, "error"); } finally { setBusy(button, false); }
  });
}
...
function showCourtModal(event = null) {
  ...
  $("#court-form").addEventListener("submit", async (submitEvent) => {
    ...
    setBusy(button, true, "Saving…");
    try {
      await api(event ? `/api/admin/events/${event.id}` : "/api/admin/events", { method: event ? "PUT" : "POST", body: new FormData(submitEvent.target) });
      modal.remove();
...
function showEventModal(event = null) {
  const modal = document.createElement("div"); modal.className = "modal-backdrop"; modal.id = "event-modal";
  ...
  $("#modal-root").append(modal); modal.querySelectorAll(".modal-close").forEach((button) => button.addEventListener("click", () => modal.remove()));
  $("#event-form").addEventListener("submit", async (e) => { e.preventDefault(); const button = e.target.querySelector("button[type=submit]"); setBusy(button, true, "Saving…"); try { const payload = Object.fromEntries(new FormData(e.target)); await api(event ? `/api/admin/events/${event.id}` : "/api/admin/events", { method: event ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); modal.remove(); toast(event ? "Event updated." : "Event created."); await openAdmin(); } catch (error) { toast(error.message, "error"); } finally { setBusy(button, false); } });
}

function showNotificationModal() {
  const modal = document.createElement("div"); modal.className = "modal-backdrop"; modal.id = "notification-modal";
  modal.innerHTML = `<div class="modal"><div class="modal-header"><div><p class="eyebrow">KEEP THE CLUB MOVING</p><h2>Send a notification</h2></div><button class="modal-close modal-close-button">×</button></div><form id="notification-form" class="modal-form"><label class="full-width">Title<input required name="title" placeholder="New court availability"></label><label class="full-width">Message<textarea required name="message" placeholder="Share an update with the RallyPoint community."></textarea></label><label class="full-width">Recipient<select name="userId"><option value="">All members</option>${state.adminData.users.filter((user) => user.role !== "admin").map((user) => `<option value="${user.id}">${esc(user.full_name)} — ${esc(user.email)}</option>`).join("")}</select></label><div class="modal-actions"><button type="button" class="button ghost modal-close-button">Cancel</button><button class="button primary" type="submit">Send update ↗</button></div></form></div>`;
  $("#modal-root").append(modal); modal.querySelectorAll(".modal-close-button").forEach((button) => button.addEventListener("click", () => modal.remove()));
  $("#notification-form").addEventListener("submit", async (event) => { event.preventDefault(); const button = event.target.querySelector("button[type=submit]"); setBusy(button, true, "Sending…"); try { await api("/api/admin/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); modal.remove(); toast("Notification sent."); await openAdmin(); } catch (error) { toast(error.message, "error"); } finally { setBusy(button, false); } });
}

document.addEventListener("click", async (event) => {
  const nav = event.target.closest("[data-page]"); if (nav) { event.preventDefault(); await navigate(nav.dataset.page); return; }
  const viewCourt = event.target.closest(".view-court"); if (viewCourt) { await showCourtDetails(Number(viewCourt.dataset.id)); return; }
  const slotStep = event.target.closest("[data-slot-date-step]"); if (slotStep && state.selectedCourt) { const nextDate = new Date(`${state.selectedSlotDate}T12:00:00`); nextDate.setDate(nextDate.getDate() + Number(slotStep.dataset.slotDateStep)); await showCourtDetails(state.selectedCourt.id, localDate(nextDate)); return; }
  const slotCell = event.target.closest(".slot-cell"); if (slotCell && !slotCell.disabled) { const slotId = Number(slotCell.dataset.slotId); state.selectedSlotIds = state.selectedSlotIds.includes(slotId) ? state.selectedSlotIds.filter((id) => id !== slotId) : [...state.selectedSlotIds, slotId]; slotCell.classList.toggle("selected", state.selectedSlotIds.includes(slotId)); const stateLabel = slotCell.querySelector("span"); if (stateLabel) stateLabel.textContent = state.selectedSlotIds.includes(slotId) ? "OPEN" : "OPEN"; updateBookingSummary(); return; }
  const confirmBooking = event.target.closest("#confirm-slot-booking"); if (confirmBooking && state.selectedCourt && state.selectedSlotIds.length) { setBusy(confirmBooking, true, "Requesting…"); try { await api("/api/slots/book", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ courtId: state.selectedCourt.id, slotIds: state.selectedSlotIds }) }); toast("Your selected slots were sent for admin approval."); await showCourtDetails(state.selectedCourt.id, state.selectedSlotDate); } catch (error) { toast(error.message, "error"); await showCourtDetails(state.selectedCourt.id, state.selectedSlotDate); } finally { setBusy(confirmBooking, false); } return; }
  const register = event.target.closest(".register-event"); if (register) { const button = register; setBusy(button, true, "Booking…"); try { await api(`/api/events/${button.dataset.id}/register`, { method: "POST" }); toast("Your court booking request was submitted."); await (state.page === "dashboard" ? renderDashboard() : renderEvents()); } catch (error) { toast(error.message, "error"); } finally { setBusy(button, false); } return; }
  const pay = event.target.closest(".pay-for-event"); if (pay) { state.page = "payments"; await renderPayments(); const select = $("#payment-form select"); if (select) select.value = state.registrations.find((item) => item.id === Number(pay.dataset.id))?.id || select.value; return; }
  const adminTab = event.target.closest("[data-admin-tab]"); if (adminTab) { state.adminTab = adminTab.dataset.adminTab; $$(".admin-tab").forEach((tab) => tab.classList.toggle("active", tab === adminTab)); renderAdminTable(); return; }
  const review = event.target.closest(".review-payment"); if (review) { let reason = ""; if (review.dataset.action === "rejected") { reason = window.prompt("Reason for rejecting this proof:") || ""; if (!reason) return; } try { await api(`/api/admin/payments/${review.dataset.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: review.dataset.action, reason }) }); toast(review.dataset.action === "verified" ? "Payment verified and registration confirmed." : "Payment rejected with reason."); await openAdmin(); } catch (error) { toast(error.message, "error"); } return; }
  const registrationApproval = event.target.closest(".registration-approval"); if (registrationApproval) { try { await api(`/api/admin/registrations/${registrationApproval.dataset.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: registrationApproval.dataset.action }) }); toast(registrationApproval.dataset.action === "approve" ? "Registration approved." : "Registration rejected."); await openAdmin(); } catch (error) { toast(error.message, "error"); } return; }
  const approval = event.target.closest(".admin-approval"); if (approval) { try { await api(`/api/admin/users/${approval.dataset.id}/admin-approval`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: approval.dataset.action }) }); toast(approval.dataset.action === "approve" ? "Admin account approved." : "Admin request rejected."); await openAdmin(); } catch (error) { toast(error.message, "error"); } return; }
  const editSchedule = event.target.closest(".edit-schedule"); if (editSchedule) { const item = state.schedules.find((schedule) => schedule.id === Number(editSchedule.dataset.id)); if (item) { const form = $("#schedule-form"); form.elements.id.value = item.id; form.elements.title.value = item.title; form.elements.scheduleDate.value = item.schedule_date.slice(0, 10); form.elements.startTime.value = item.start_time.slice(0, 5); form.elements.endTime.value = item.end_time ? item.end_time.slice(0, 5) : ""; form.elements.location.value = item.location; form.elements.notes.value = item.notes || ""; $("#cancel-schedule-edit").classList.remove("hidden"); form.querySelector("button[type=submit]").textContent = "Update schedule ↗"; form.scrollIntoView({ behavior: "smooth", block: "start" }); } return; }
  const deleteSchedule = event.target.closest(".delete-schedule"); if (deleteSchedule && window.confirm("Delete this playing schedule?")) { try { await api(`/api/schedules/${deleteSchedule.dataset.id}`, { method: "DELETE" }); toast("Schedule deleted."); await renderSchedule(); } catch (error) { toast(error.message, "error"); } return; }
  const edit = event.target.closest(".edit-event"); if (edit) { showCourtModal(state.adminData.events.find((item) => item.id === Number(edit.dataset.id))); return; }
  const remove = event.target.closest(".delete-event"); if (remove && window.confirm("Delete this event and its registrations?")) { try { await api(`/api/admin/events/${remove.dataset.id}`, { method: "DELETE" }); toast("Event deleted."); await openAdmin(); } catch (error) { toast(error.message, "error"); } }
});

document.addEventListener("change", async (event) => {
  if (event.target.id !== "slot-date" || !state.selectedCourt || !event.target.value) return;
...
$("#admin-login-form").addEventListener("submit", (event) => handleAuth(event, "admin"));
$("#register-form").addEventListener("submit", (event) => handleAuth(event, "register"));
$("#admin-register-form").addEventListener("submit", (event) => handleAuth(event, "admin-register"));
...
$$(".auth-tab").forEach((tab) => tab.addEventListener("click", () => switchAuth(tab.dataset.authTab)));
$("[data-auth-tab='forgot']").addEventListener("click", () => switchAuth("forgot"));
$$(".back-to-login").forEach((button) => button.addEventListener("click", () => switchAuth("login")));
...
$$(".auth-menu-item").forEach((item) => item.addEventListener("click", () => {
...
(async function start() {
  ...
  try { const data = await api("/api/auth/me"); if (data.user) { state.user = data.user; await bootApp(); } } catch (error) { console.error(error); }
})();