...
async function renderPayments() {
  ...
  $("#payment-form")?.addEventListener("submit", async (event) => {
    ...
    try {
      await api("/api/payments", { method: "POST", body: new FormData(event.target) });
      toast("Payment proof submitted successfully. It is now pending admin review.");
...
document.addEventListener("click", async (event) => {
  ...
  const pay = event.target.closest(".pay-for-event"); if (pay) { state.page = "payments"; await renderPayments(); const select = $("#payment-form select"); if (select) { select.value = state.registrations.find((item) => item.id === Number(pay.dataset.id))?.id || select.value; select.dispatchEvent(new Event("change")); } return; }
